...
export function createGeminiTranscriptionService() {
  return {
    async transcribe({ filePath, mimeType }) {
      if (getGeminiApiKeys().length === 0) throw createAppError('TRANSCRIPTION_CONFIG_ERROR', 'GEMINI_API_KEY is not configured');
      ...
      return withGeminiTransientRetry(
        async (apiKey) => {
          if (!apiKey) throw createAppError('TRANSCRIPTION_CONFIG_ERROR', 'GEMINI_API_KEY is not configured');
          ...
          let response;
          try {
            response = await fetchWithTimeout(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': apiKey }, body: JSON.stringify(body) }, env.transcriptionTimeoutMs);
          } catch (err) {
          ...
          if (!response.ok) {
            ...
            if (status === 503) {
              throw createAppError('AI_PROVIDER_OVERLOADED', `Transcription provider overloaded: ${apiMessage}`, { statusCode: 503 });
            }
            if (status === 404) {
