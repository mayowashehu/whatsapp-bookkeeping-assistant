...
export async function routeByIntent({ intent, text, fromNumber, knownProperties, senderId, allowFastPath = true, context = {}, activeDraft = null }) {
  ...
  if (intent === 'CONFIRMATION') {
    ...
    // BUG FIX (manual WhatsApp testing, 🔴 — confirmed live): everything
    // above this point already disambiguates polarity itself (their own
    // NEGATIVE_CONFIRMATION_WORDS checks happen before intent classification
    // even runs, further up in resolvePipelineResult). This final fallback
    // — the plain PendingDraft path, most notably the duplicate-warning
    // prompt — did not, and unconditionally called handleConfirmation
    // regardless of what the message actually said. A bare "No" reaching
    // this branch (e.g. via a voice note transcript, or any other caller of
    // routeByIntent that doesn't go through resolvePipelineResult's own
    // activeDraft short-circuit) got saved instead of cancelled. See the
    // matching, primary fix in resolvePipelineResult for the main text-path
    // case; this is the defense-in-depth layer for every other caller.
    if (NEGATIVE_CONFIRMATION_WORDS.test(text.trim())) {
      return DraftManager.handleCancel({ fromNumber: cleanFrom, knownProperties });
    }
...
export async function resolvePipelineResult({ content, cleanFromNumber, senderId, knownProperties, activeDraft }) {
  ...
  if (activeDraft) {
    // BUG FIX (manual WhatsApp testing, 🔴 — confirmed live, REAL DATA
    // INTEGRITY BUG): replying "No" to a pending draft — most critically
    // the duplicate-warning prompt ("...Reply YES to save it anyway, or
    // NO to cancel this one") — used to get SAVED anyway instead of
    // cancelled. Root cause: nothing in this function ever checked for a
    // bare negative ("no"/"n") against an active draft at all — only the
    // literal cancel-word list (isCancelCommand, checked earlier, doesn't
    // include "no") and the positive CONFIRMATION_WORDS check below were
    // handled. A bare "No" fell through this entire function untouched,
    // reached classifyMessage, got classified with intent 'CONFIRMATION'
    // (that classifier bucket doesn't encode polarity), and
    // routeByIntent's CONFIRMATION branch unconditionally calls
    // DraftManager.handleConfirmation — i.e. treats ANY confirmation-shaped
    // reply as a yes. This is the exact bug: a user explicitly declining
    // ("No, cancel this one") had their entry saved anyway. Checked first,
    // before the clarification-answer branch, so it applies no matter what
    // state the draft is in (mirrors how isCancelCommand is checked
    // unconditionally too).
    if (NEGATIVE_CONFIRMATION_WORDS.test(content.trim())) {
      const result = await DraftManager.handleCancel({ fromNumber: cleanFromNumber, knownProperties });
      await PendingStatement.deleteOne({ fromNumber: cleanFromNumber }).catch(() => {});
      return { ...result, replyText: result?.replyText || 'Discarded. Nothing was saved.', classification: 'CANCEL' };
    }
